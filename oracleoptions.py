...
import pandas as pd
import log
logg = log.log('oracleoptions')



def sqlSelect(sql, db):
    '''查询'''
    logg.saveInfo('查询:' + sql)
    cr = db.cursor()
    cr.execute(sql)
    rs = cr.fetchall()
    cr.close()
...
def save_to_oracle_var(df, database=''):
    logg.saveInfo('以字符串形式保存到数据库' + database)
    # 创建日志文件
...
def save_to_oracle_int(df, database=""):
    logg.saveInfo('以整数形式保存到数据库' + database)
    conn = cx_Oracle.connect("star/star@127.0.0.1/orcl")
    cursor = conn.cursor()  # 创建游标对象
